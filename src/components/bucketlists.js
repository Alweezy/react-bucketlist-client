...
export class BucketLists extends React.Component {
    ...
    componentWillMount() {
        ...
        axios.get('https://buck3tlist.herokuapp.com/api/v1/bucketlists/',
    ...
    handleNewBucketList(e) {
        e.preventDefault();
        let access_token = 'Bearer ' + localStorage.getItem('access_token');
        axios.post('https://buck3tlist.herokuapp.com/api/v1/bucketlists/', JSON.stringify({'name': this.refs.name.value}),
    ...
    handleEditBucketList(e) {
        e.preventDefault();
        let access_token = 'Bearer ' + localStorage.getItem('access_token');
        axios.put('https://buck3tlist.herokuapp.com/api/v1/bucketlists/', JSON.stringify({'name': this.refs.name.value}),
            {headers: {'Authorization': access_token, 'Content-Type': "application/json"}}
        )
            .then((response) => {
                window.location.reload();
    ...
    handleNewItem(e) {
        e.preventDefault();
        let access_token = 'Bearer ' + localStorage.getItem('access_token');
        axios.delete('https://buck3tlist.herokuapp.com/api/v1/bucketlists/', JSON.stringify({'name': this.refs.name.value}),
            {headers: {'Authorization': access_token, 'Content-Type': "application/json"}}
        )
            .then((response) => {
                window.location.reload();
